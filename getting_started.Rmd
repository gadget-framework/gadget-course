...
library(Rgadget)
```
For convenience Rgadget will also load the tidyverse package as it is used extensively when interacting with Gadget. We will introduce the functions in Rgadget as we go through the tutorial. Now we will focus on setting up a simple dummy model using the `gadgetfile` function and related functions. `gadgetfile` creates new files in a format that coheres to the Gadget file format by using lists embedded within lists.
```{r}
gadgetfile('test',file_type = 'generic', 
           components = list(a=list(bla=1),
                             b=list(ble=1)))
```
The file type argument allows function to customize the output depending to the various quirks of the file. The output can then subsequently be written to a file using `write.gadget.file`. 
...
Rgadget has a number of convenience functions when dealing with more complicated files. These functions are all derivatives of `gadgetfile` and thus can be written to file using `write.gadget.file`. We will start off by creating a skeleton stock file:
...
We can of course do multiple edits using `gadget_update` in one go:
...
This essentially creates a stock with 1000 fish that do not grow, consume or die for the duration of the simulation. Behind the scenes R has created a folder named `simple_model` with all the necessary files:
```{r}
fs::dir_ls(gd)
```
