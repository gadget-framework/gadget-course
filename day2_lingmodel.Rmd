...
Also notice that although it is necessary to define the arguments defined above (minage,
maxage, minlength, maxlength, dl, and livesonareas), updating the 'stock' component with
gadget_update also sets up a few more convenient default settings in the gadget stock file.
...
Now, initial conditions need to be set up for each stock. For example, the immature stock needs
11 age bins x 35 length bins to contain initial numbers of individuals. Gadget is set up to put
10,000 fish in each age group per area, but to be distributed across the 35 length bins, they
also need to be assigned lengths.  Lengths are distributed by a normal distribution, to which a 
mean per age is supplied by the user and an optional overal standard deviation. [Alternatively,
it is possible to supply the numbers that make up the stock distribution, but implementation of 
this method should be referred to in the Gadget User Guide.]

Yesterday, we saw that these can be set very simply as a number, but that we can also replace
numbers with parameters and even formulas. Today we will use this capability to set the initial 
conditions of the ling model to have two more realistic properties. We know 1) that there should
be fewer older individuals than younger individuals, corresponding to the total mortality 
experienced by the population at the beginning of the time series, and 2) older individuals 
should have larger bodies than younger individuals, as specified by a Von Bertalanffy relationship. 

To update the initial conditions to reflect these patterns, we use gadget_update, followed by 
the component title `'initialconditions'`, and the key normalparam that will be updated with a 
data frame. The first column is for age, which includes a sequence of those ages already specified within ling.imm. [That is, using the `.` placeholder passes the ling.imm object to that location, where minage and maxage information is extracted]. The second column assigns all ages to area 1. The column `age.factor` contains code that creates a different formula for each age. It 

It also shows how to translate
lengths into weights via a constant length-weight key, provided under the `'refweight'` 
component, which is the same for both stocks.
```{r}
