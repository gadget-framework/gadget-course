...
Recruitment can be thought of as another type of Initial Condition. In addition to numbers-at-age and their length distributions being needed for all ages in the first time 
...
the immature stocks requires inputted individuals from a recruitment process. The process 
could bemodeled as a stock-recruitment relationship, but for ling we actually keep it more
simple and simply provide switches for each year's number at the youngest age. That way, 
these can either be inputted as values through a params file or they can be estimated at a
later stage. 

To incorporate recruitment, `gadget_update` is used to switch on the `doesrenew` argument to
...
recruitment occurs. An area and age combination need to be provided as in initial conditions,
...
'number' that scales these 10,000 fish up or down relative to other among year x timestep
combinations. Below, we have provided a knew parameter for each year (beginning with
`'ling.rec'`), each of which are multiplied by the scalar `'ling.rec.scalar'` which can be
used to scale the whole series. Here, lengths are supplied via a normal parametric 
...
represents a length at age 0. In the last columns, the same `'ling.walpha'` and `'ling.wbeta'` 
...
```{r}
 ling.imm <-
  ...
  gadget_update('doesrenew',
                normalparam = data_frame(year = schedule$year,
                                         step = 1,
                                         area = 1,
                                         age = .[[1]]$minage,
...
### Set up fleets using catch data

First, catch data must be organized in the same structure as the model (units kg). Here we 
have three fleets, longline, bottom trawl, and gillnet fleets. They are separated by gear so
that a different selectivity curve can be implemented for each fleet. The survey is also
...
the population, we 'trick' Gadget by only allowing it to have a 1 kg catch in within each 
step / year combination. There will need to exist catch in these steps because later in the
tutorial, we will declare that biological data came from these catches.

Also notice that the attribute `'area_group'` need to be applied to the data frame using the 
function `structure`. These attributes are called on by Rgadget when creating the
appropriate aggregate files.

```{r}

#WILL THIS WORK WITHOUT MFDB GROUP?
...
#Something weird about the levels of area_group list here
lln.landings <- structure(read_csv('data_provided/lln.csv'),
            area_group = list(`1` = 1))
bmt.landings <- structure(read_csv('data_provided/bmt.csv'),
            area_group = list(`1` = 1))
...
`'linearfleet'` or `'efffortfleet'` which are implementations that control effort. The 
...
`'totalfleet'`), giving the fleet a name (below 'surv', 'lln','bmt', and 'foreign'),
...
differences in data and other settings particular to fleets. 
...
Now we run just as in the simple model example. Here we chose to set the directory to model
location, then use `Sys.setenv` to set the Gadget working directory to the model path, and
finally call Gadget to do a simple simulation.

```{r, echo = FALSE, root.dir = 'ling_model/01-base'}
...
Sys.setenv(GADGET_WORKING_DIR=normalizePath(gd))
getwd()
callGadget(s=1,main='main',ignore.stderr = FALSE,log = 'simple_log2', i='../../data_provided/params.forsim')
