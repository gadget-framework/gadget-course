...
This will start an iterative run that will estimate the weights for all likelihood components, although penalising likelihood components, such as `boundlikelihood` and `understocking` are left as is, and run a final optimisation run with the resulting likelihood weigths. As the iterative reweighting requires a number of optimisation runs `gadget.iterative` tries to spread these runs across the available cores on your computer. In the case of the number of cores lower than the number of runs the excess runs will wait until a core becomes available. The resulting parameter estimates are then stored in a folder called `WGTS` in your model directory, where the file `params.final` contains the final parameter estimate.  

When running `gadget.iterative` with standard setting you assume that all datasets has enough data that allows all parameters to be estimated with out overfitting the data. It is not always the case that this is appropriate. For instance survey indices have in general only one data point per year, step and size group, and when estimating annual recruitment this usually leads overfitting of that particular component when heavily emphasized. Similar problem can arise when only one year, or few disjoint years, worth of compositional data is supplied to the likelihood function. This account for this issue the user can group similar likelihood components together using the `grouping` switch in the `gadget.iterative` function. This grouping the empasizes two (or more) components together, ensuring that at least two data points are available for each of the recruiment parameters. In the case of Ling, the survey indices were split into two groups, sizes 20 to 70 cm and 70 to 160 cm, and the compositional data from the commercial trawl and gillnet fleets due to lack of data:
```{r eval=FALSE}
...
Note that if a likelihood component is not present in the defined grouping list it is assumed that the component is treated individually. Sometimes this is not enough to prevent overfitting and therefore the user can define a maximum weight of sorts by defining the minimum CV for survey indices using the `cv.floor` argument:

```{r eval=FALSE}
run <- gadget.iterative(cv.floor = 0.2)
```

The `cv.floor` argument is often used after the final optimisation run has crashed so it is convenient to be able to pick up the "thread" where the run crashed and resume the final optimisation. The `resume.final` switch allows for this and essentially only runs the last optimisation, assuming that the precending steps have been completed:

```{r eval=FALSE}
run <- gadget.iterative(resume.final = TRUE, cv.floor = 0.2)
```

Related to `resume.final` there are the `run.final` and `run.serial` that instruct the function to whether to run the final step and run on a single core respectively. 

Different treatment of data is often investigated, e.g. investigate the effect of a certain dataset by omitting from the estimation or different grouping. `gadget.iterative` allows you to have multiple `WGTS` folders in the same model directory by setting the `wgts` argument. In addition the user can explictly define (or remove) which component are to be used in the optimisation using a combination of the `comp` and `inverse` arguments:
...
To obtain information on the model fit and properties of the model one can use the `gadget.fit` function to query the model:
